...
# 功能：配置全局 bash/zsh 命令日志记录，记录所有用户的操作历史到 /var/log/shell.log
...
chmod 777 /var/log/shell.log
chown root:root /var/log/shell.log
...
record_global_command() {
    ...
    # 写入日志（仅root可写，避免篡改）
    echo "[$timestamp] [USER:$user] [IP:$ip] [TTY:$tty] [EXIT:$exit_code] [HIST:$hist_num] $last_cmd" | sudo tee -a /var/log/shell.log > /dev/null 2>&1
}
...
# 配置全局zsh日志钩子（/etc/zsh/zshrc 或 /etc/zshrc）
echo "配置全局zsh日志记录..."
zshrc_path=""
if [ -f "/etc/zsh/zshrc" ]; then
    ...
    echo "警告：未找到全局zsh配置文件，跳过zsh配置" >&2
fi

if [ -n "$zshrc_path" ]; then
    if ! grep -q "record_global_zsh_command()" "$zshrc_path"; then
        cat << 'EOF' >> "$zshrc_path"

record_global_zsh_command() {
    ...
    if [ "$hist_num" = "$ZSH_LAST_HIST_NUM" ]; then
        return  # 编号相同，说明是重复触发，跳过
    fi
    export ZSH_LAST_HIST_NUM="$hist_num"  # 更新为最新编号
...
# 确保precmd_functions中只添加一次函数（防重复）
if ! (( ${precmd_functions[(I)record_global_zsh_command]} )); then
    precmd_functions+=(record_global_zsh_command)
fi
EOF
    else
        echo "全局zsh日志记录函数已存在，跳过添加"
    fi
fi

# 配置日志轮转
echo "配置日志轮转..."
logrotate_conf="/etc/logrotate.d/shelllog"
...
echo "配置完成！新的bash/zsh会话将开始记录命令日志到/var/log/shell.log"
